...
export class SequentialTurnManager3P<TCellType extends Enumerable, TUnitType extends Enumerable, TUnitOwner extends Enumerable> 
  ...
  constructor(game: Game<TCellType, TUnitType, TUnitOwner>) {
    ...
    super(game, {
      ...
      nextTurnOwnerFn: (current) => {
        ...
        return game.initialConfig.unitOwners[(index + 1) % 3];
      },
  ...
  copy(source: TurnManager<TCellType, TUnitType, TUnitOwner>): TurnManager<TCellType, TUnitType, TUnitOwner> {
    if (!(source instanceof SequentialTurnManager3P)) {
